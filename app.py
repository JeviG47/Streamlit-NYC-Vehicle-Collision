...
st.title("Motor Vehicle Collisions in NYC 2024")
st.markdown("This application is a Streamlit dashboard that can be used to analyze motor vehicle collisions in NYC")

def load_data(nrows):
    data = pd.read_csv("data/Motor_Vehicle_Collisions_-_Crashes_20240210.csv", nrows=nrows, parse_dates=[['CRASH DATE', 'CRASH TIME']])
...
st.markdown("Vehicle collisions  between %i:00 and %i:00" %(hour,hour))
